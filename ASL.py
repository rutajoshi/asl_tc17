...
while(cap.isOpened()):
    ...
    im, contours, hierarchy = cv2.findContours(mask.copy(),cv2.RETR_EXTERNAL, 2)
    cnt = util.getMaxContour(contours,4000) # using contours to capture the skin filtered image of the hand
    if cnt != None:
